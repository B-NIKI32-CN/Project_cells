...
while running:
    ...
    if scene == 'menu':
        ...
        if players_flag:   # регистрация игроков
            ...
            player = players[turn]
            players_flag = False
            mist_map = np.zeros((map_len_cells, map_len_cells))
        ...
        if keys_clicked[32] == 1 and open_win_market == 0 and ready_to_spawn_tank != False and player.base != 0: # установка выбранного танка
            ...
            if dist_spawn0 <= dist_spawn:
                if player.res >= ready_to_spawn_tank.ttx[-3]:
                    ...
                    player.mists.empty()
                    mist_map = scripts.functions.mist_doting(np.zeros((map_len_cells, map_len_cells), np.int64),player.tanks)
                    scripts.functions.mist_builder(mist_map, scripts.mist.Mist, player.mists)
                ready_to_spawn_tank = False
        ...
        if player.base == 0: # установка базы игрока
            ...
            if (keys_clicked[32] == 1 and 0<=cell_mouse_pos[0]<map_len_cells and 0<=cell_mouse_pos[1]<map_len_cells
                ...
                player.mists.add(scripts.mist.Mist(cell_mouse_pos[0], cell_mouse_pos[1]))
        ...
        all_projectiles.update(all_walls, all_tanks, player.tanks)
        ...
        for tank in all_tanks:
            if mist_map[tank.place[1], tank.place[0]] > 0:
                tank.draw(virtualscreen)
        # all_bases.draw(virtualscreen)
        for base in all_bases:
            if mist_map[base.place[1], base.place[0]] > 0:
